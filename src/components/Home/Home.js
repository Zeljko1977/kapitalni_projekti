...
const Input = (props) => {
  ...
  const addProjectHandler = async (event) => {
    ...
    if (
      enteredNaziv.trim().length === 0 ||
      enteredBudzet.trim().length === 0 ||
      enteredOpis.trim().length === 0 ||
      enteredCilj.trim().length === 0
    ) {
      setError({
        title: "Invalid input",
      ...
      <div className="test">
        ...
        <form onSubmit={addProjectHandler}>
          ...
          <input
            ...
            className="question"
            id="nme"
            onChange={budzetChangeHandler}
            autoComplete="off"
          />
          <label htmlFor="nme">
            <span>Budzet</span> <Valute />
          </label>
          ...
          <textarea
            ...
            className="question"
            id="msg"
            onChange={ciljChangeHandler}
            autoComplete="off"
          ></textarea>
          <label htmlFor="msg">
            <span>Cilj projekta</span>
          </label>

          <Button onDodajProjekat={addInputHandler} className="button , input" type="submit">
            Dodajte projekat
          </Button>
